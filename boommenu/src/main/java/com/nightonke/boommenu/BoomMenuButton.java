...
public class BoomMenuButton extends FrameLayout implements InnerOnBoomButtonClickListener {
    ...
    private void placePiecesAtPositions() {
        int pieceNumber = pieceNumber();
        for (int i = 0; i < pieceNumber; i++) {
            if(pieces.size() >= pieceNumber && piecePositions.size() >= pieceNumber)
    ...
    public Point getCoordinateOrigin(){
        if(background == null)
            createBackground();
        return new Point(background.getLayoutParams().width,background.getLayoutParams().height);
    }
    ...
    private void startBoomAnimations(boolean immediately) {
        ...
        for (int i = indexes.size() - 1; i >= 0; i--) {
            ...
            BoomButton boomButton1 = boomButtonBuilders.get(index).build(context);
            boomButtons.set(index, boomButton1);
    ...
    public void calculateStartPositions(boolean force) {
    ...
    public void setUserocl(OnClickListener userocl) {
        this.userocl = userocl;
    }
