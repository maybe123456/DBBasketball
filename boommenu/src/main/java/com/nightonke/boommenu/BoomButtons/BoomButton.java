...
public abstract class BoomButton extends FrameLayout {
    ...
    protected int buttonRadius;  // for simple/inner_tv-inside/inner_tv-outside circle button
...
//    protected boolean isImgGone = false;
    ...
    // Text for inner_tv-outside-circle-button
...
//        isImgGone = builder.isImgGone;
        ...
        // for inner_tv-outside-circle-button
...
//        image.setVisibility(isImgGone ? View.GONE : View.VISIBLE);
    }

    protected void initText(ViewGroup parent) {
        inner_tv = button.findViewById(R.id.inner_tv);
        updateText();
        toNormalText();
        inner_tv.setText(normalText);
//        updateTextRect();
//        updateTextPadding();
        if (typeface != null) inner_tv.setTypeface(typeface);
//        inner_tv.setMaxLines(maxLines);
//        inner_tv.setTextSize(TypedValue.COMPLEX_UNIT_SP, textSize);
//        inner_tv.setGravity(textGravity);
//        inner_tv.setEllipsize(ellipsize);
//        if (ellipsize == TextUtils.TruncateAt.MARQUEE) {
//            inner_tv.setSingleLine(true);
//            inner_tv.setMarqueeRepeatLimit(-1);
//            inner_tv.setHorizontallyScrolling(true);
//            inner_tv.setFocusable(true);
//            inner_tv.setFocusableInTouchMode(true);
//            inner_tv.setFreezesText(true);
//        }
//        parent.addView(inner_tv);
    }
...
//        if (inner_tv != null) inner_tv.setLayoutParams(params);
    }

    void updateTextPadding() {
//        if (textPadding != null && inner_tv != null) {
//            inner_tv.setPadding(
//                    textPadding.left,
//                    textPadding.top,
//                    textPadding.right,
...
//        else Util.setDrawable(image, highlightedImageRes, highlightedImageDrawable);
    ...
    protected void toNormalText() {
        if (unable) {
            Util.setText(inner_tv, unableTextRes, unableText);
            Util.setTextColor(inner_tv, unableTextColorRes, unableTextColor);
        } else {
            Util.setText(inner_tv, normalTextRes, normalText);
            Util.setTextColor(inner_tv, normalTextColorRes, normalTextColor);
        }
    }
    ...
    protected void toHighlightedText() {
        if (unable) {
            ...
            Util.setText(inner_tv, highlightedTextRes, highlightedText);
            Util.setTextColor(inner_tv, highlightedTextColorRes, highlightedTextColor);
        }
    }
    ...
    public void didShow() {
        ...
        if (!rippleEffectWorks && isNeededColorAnimation())
            Util.setDrawable(button, nonRippleBitmapDrawable);
        if (inner_tv != null) inner_tv.setSelected(true);
    ...
    public void didHide() {
        if (inner_tv != null) inner_tv.setSelected(false);
...
//    }
    ...
    /**
     * Get the inner_tv view of a boom button.
     *
     * @return inner_tv view
     */
    ...
    /**
     * Get the sub inner_tv view of a boom button.
     *
     * @return sub inner_tv view
     */
